 ...
 * 	@return Number of nodes removed. */
	...
	/** Hilfsfunktion f¸r sort, vertauscht in einem Array zwei Elemente. */
 ...
 * 	@return Residual graph. */
	...
	public static Graph buildSeperatorTree(Graph g) {
		...
		// In folgendem Array wird zu jedem Knoten derjenige Knoten gespeichert, an den er im Seperator Tree angekn¸pft werden muﬂ. Genauer: Am Anfang muﬂ jeder Knoten nat¸rlich an den einzigen, bereits im Baum vorhandenen Knoten angef¸gt werden. Dann bekommt jeder zum Baum hinzugef¸gte Knoten diejenigen Knoten vom alten "¸berwiesen", die auf seiner Seite des minimalen Schnittes liegen.
		...
		for (int i = 1; i < nodeCount; i++) {
			// Der Knoten n ist noch nicht im Baum enthalten, wird im folgenden hinzugef¸gt, und zwar muﬂ er an m angef¸gt werden:
			...
			// Der Residualgraph zum maximalen Fluﬂgraphen reicht, um den minimalen Schnitt zu bekommen; Knoten auf m's Seite des Schnitts sind im Residualgraph nicht von n aus erreichbar, haben in diesem Array also Distanz -1:

			Graph resGraph = maxFlow(g, n, m);
			int[] rDist = resGraph.labelDist(nID);

			// Alle Knoten, die vorher zu m gehˆrten, aber auf n's Seite des Schnittes liegen, gehˆren jetzt zu n:

			for (int j = 0; j < part.length; j++)
				if ((part[j] == m) && (rDist[j] >= 0))
					part[j] = n;

			// Der Wert des minimalen Schnitts muﬂ berechnet werden:
